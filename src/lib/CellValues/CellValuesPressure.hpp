...
template <int dim>
class CellValuesPressure : public CellValuesBase<dim>
{
 public:
  CellValuesPressure(const Model::Model<dim> &model_);
...
template <int dim>
CellValuesPressure<dim>::CellValuesPressure(const Model::Model<dim> &model)
    :
...
template <int dim>
void
CellValuesPressure<dim>::update(const CellIterator<dim> & cell,
                                const SolutionValues    & solution_values)
{
  ...
  this->delta_div_u = solution_values.div_u - solution_values.div_old_u;
  ...
  if (model.has_phase(Model::Phase::Water))
  {
    if (model.n_phases() > 1)
    {
      this->Sw = solution_values.saturation[0];
  ...
  if (model.has_phase(Model::Phase::Oil))
  {
    if (model.fluid_model == Model::FluidModelType::DeadOil)
      ...
      this->So = solution_values.saturation[1];
...
template <int dim>
void
CellValuesPressure<dim>::update_wells(const CellIterator<dim> &cell)
{
  vector_J_phase = 0;
  vector_Q_phase = 0;
...
inline
double
CellValuesPressure<dim>::get_matrix_face_entry() const
{
  double entry = 0;
  if (model.fluid_model == Model::FluidModelType::Liquid)
    entry += T_w_face;
...
inline
double
CellValuesPressure<dim>::get_rhs_face_entry(const double,
                                            const int) const
{
  double entry = 0;
  if (model.fluid_model == Model::FluidModelType::Liquid)
